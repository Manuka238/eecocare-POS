...
export async function POST(req: NextRequest) {
  try {
    ...
    // Validate Required Columns for Supplier
    if (contentType.includes("multipart/form-data")) {
      const hasName = matchedCanonicalKeys.has("display_name") || matchedCanonicalKeys.has("business_name") || matchedCanonicalKeys.has("full_name");
      const hasPhone = matchedCanonicalKeys.has("mobile_number");
      if (!hasName) {
        return NextResponse.json({ success: false, error: "Missing required column: Supplier Name" }, { status: 400 });
      }
      if (!hasPhone) {
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      const finalId = row.contact_id || row.id || `SUP-IMP-${Math.floor(100000 + Math.random() * 900000)}`;
